...
<article class="torrent-search--grouped__result">
    <header class="torrent-search--grouped__header">
        @if (auth()->user()->show_poster == 1)
            <a href="{{ route('torrents.similar', ['category_id' => $media->category_id ?? '', 'tmdb' => $media->id ?? '']) }}" class="torrent-search--grouped__poster">
                <img src="{{ isset($media->poster) ? tmdb_image('poster_small', $media->poster) : 'https://via.placeholder.com/90x135' }}" alt="{{ __('torrent.poster') }}" loading="lazy" />
            </a>
        @endif
        ...
        <address class="torrent-search--grouped__directors">
            @if (!empty($media->creators) && $media->creators->isNotEmpty())
                <span class="torrent-search-grouped__directors-by">by</span>
                @foreach ($media->creators as $creator)
                    <a href="{{ route('mediahub.persons.show', ['id' => $creator->id ?? '', 'occupationId' => App\Enums\Occupation::CREATOR->value]) }}" class="torrent-search--grouped__director">
                        {{ $creator->name ?? '' }}
                    </a>
                    @if (!$loop->last),@endif
                @endforeach
        ...
        <div class="torrent-search--grouped__genres">
            @foreach ($media->genres->take(3) ?? [] as $genre)
                <a href="{{ route('torrents.index', ['view' => 'group', 'genres' => $genre->id ?? '']) }}" class="torrent-search--grouped__genre">
                    {{ $genre->name ?? '' }}
                </a>
            @endforeach
        </div>
    ...
    <section>
        @if (!empty($media->torrents) && $media->torrents->has('Complete Pack'))
                ...
                <table class="torrent-search--grouped__torrents">
                    ...
                    @foreach ($media->torrents['Complete Pack'] ?? [] as $type => $torrents)
                        @foreach ($torrents ?? [] as $torrent)
                            <tr>
                                @if ($loop->first)
                                    <th class="torrent-search--grouped__type" scope="rowgroup" rowspan="{{ count($torrents) }}">
                                        {{ $type ?? '' }}
                                    </th>
                                @endif
        ...
        @if (!empty($media->torrents) && $media->torrents->has('Specials'))
            <details
        ...
        @foreach ($media->torrents['Seasons'] ?? [] as $seasonName => $season)
            ...
            >
                ...
                @if (!empty($season) && $season->has('Season Pack'))
                ...
                @foreach ($season['Episodes'] ?? [] as $episodeName => $episode)
                    <details
                        class="torrent-search--grouped__dropdown"
                        @if ($loop->last && (!isset($season) || !$season->has('Season Pack')))
                            open
                        @endif
                    >
