        ...
        <div class="panel__body">
            ...
            @else
                <p class="form__group form__group--horizontal">
                    ...
                    <a class="form__button form__button_copy--filled form__button--centered" style="margin-left: 24px;" x-data data-link="{{ route('torrent.download.rsskey', ['id' => $torrent->id, 'rsskey' => auth()->user()->rsskey]) }}" x-on:click.stop="navigator.clipboard.writeText($el.getAttribute('data-link')).then(() => Swal.fire({toast: true, position: 'top-end', showConfirmButton: false, timer: 3000, icon: 'success', title: 'Copy to clipboard'}))">
                        <i class="{{ config('other.font-awesome') }} fas fa-link"></i> {{ __('common.copydownloadlink') }}
                    </a>
                </p>
            @endif
        </div>
