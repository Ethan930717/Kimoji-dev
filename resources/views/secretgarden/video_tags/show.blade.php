...
@section('title')
    <title>{{ $tag->name }} - {{ config('other.title') }}</title>
@endsection
...
@section('breadcrumbs')
    ...
    <li class="breadcrumb--active">
        <a class="breadcrumb__link" href="{{ route('secretgarden.video_tags.index') }}">
    ...
    <section class="panelV2">
        <header class="panel__header">
            <div style="display: flex; align-items: center; margin-top: 10px;" >
                <h2 class="panel__heading">{{ $tag->name }} ({{ $videos->total() }})</h2>
                <div style="display: inline-flex; align-items: center; margin-left: 10px;">
                    <a href="{{ route('secretgarden.video_tags.show', ['id' => $tag->id, 'sort' => 'release_date', 'direction' => $sortDirection === 'asc' ? 'desc' : 'asc']) }}" title="Sort by release date" style="margin-right: 10px;">
                        <i class="fa fa-calendar{{ $sortField === 'release_date' ? ($sortDirection === 'asc' ? ' up' : ' down') : '' }}"></i>
                    </a>
                    <a href="{{ route('secretgarden.video_tags.show', ['id' => $tag->id, 'sort' => 'video_rank', 'direction' => $sortDirection === 'asc' ? 'desc' : 'asc']) }}" title="Sort by rank">
